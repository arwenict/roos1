...
class TigraMenu {
	...
	function loadSubContent($item=null,$subItem=null,$level=0,$ret=true) {
		$data='';
		if (!$item->megaparams->get('group')) $data .= '<ul class="tc-menu level-' . $level . '"><li class="menu-item group"><div class="group"><div class="group-content">';
	...
	function beginMegaSub($pid=0, $level=0, $pos, $i, $ret=false){
		...
		if (@$this->children[$pid]) $data .= '<ul class="tc-menu level-' . $level . '">';//modified
		if ($ret) return $data; else echo $data;
	}
					...
					if (@$this->children[$pid]) {
						foreach ($this->children[$pid] as $row) {
							if (@$this->children[$row->id]) {
								...
								echo "<ul class=\"tc-menu empty level-1\"><li class=\"empty\">&nbsp;</li></ul>";
							}
						}
					}
				}	
