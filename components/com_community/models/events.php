...
CFactory::load( 'tables' , 'memberlist' );
...
implements CGeolocationSearchInterface , CNotificationsInterface
{
	...
	public function getTotalGroupEvents( $groupId )
	{
		...
		$extraSQL = ' AND ((' . $db->nameQuote('parent') .'=' . $db->quote(0) . ' && (' . $db->nameQuote('repeat') .'=' . $db->quote('') . ' || ' . $db->nameQuote('repeat') . ' IS NULL)) || (' . $db->nameQuote('parent') . '!= 0 && ' . $db->nameQuote('repeat') . ' IS NOT NULL))';
	...
	{ 
		...
		if( !empty($categoryId) && $categoryId != 0 )
		{
                        if (is_array($categoryId)) {
                            ...
                            $extraSQL   .= ' AND a.' . $db->nameQuote('catid'). ' IN(' . $categoryIds . ')';
                        } else {
		...
		if( !empty($advance) )
		{
			...
			{
				$extraSQL	.=  ' AND a.' . $db->nameQuote('parent').' = ' . $db->Quote( $advance['parent'] );
			...
			{       if (is_array($advance['id']) && count($advance['id']) > 0) {
                                    ...
                                    $extraSQL	.=  ' AND a.' . $db->nameQuote('id').'IN (' . $ids . ')';
                                }
			}
		...
		if( $type != CEventHelper::ALL_TYPES )
		{
			$extraSQL   .=  ' AND a.' . $db->nameQuote('type').'=' . $db->Quote( $type );
                        ...
                        if (is_array($contentid)) {
                            ...
                            $extraSQL   .=  ' AND a.' . $db->nameQuote('contentid'). ' IN(' . $contentids . ')';
                        } elseif ($contentid > 0){
		...
		// Filtering on repeat event's parent
		$extraSQL .= ' AND ((a.' . $db->nameQuote('parent') .'=' . $db->quote(0) . ' && (a.' . $db->nameQuote('repeat') .'=' . $db->quote('') . ' || a.' . $db->nameQuote('repeat') . ' IS NULL)) || (a.' . $db->nameQuote('parent') . '!= 0 && a.' . $db->nameQuote('repeat') .' IS NOT NULL))';
	...
	public function getEventLastChild($parent)
	{
		...
		$query	= 'SELECT id'
				...
				. ' WHERE ' . $db->nameQuote('parent').'=' . $db->Quote( $parent ) 
				. ' ORDER BY id DESC'
				. ' LIMIT 1';
	...
	public function isEventExist( $title, $location, $startdate, $enddate, $id=0, $parent=0 ) {
		...
		$strSQL	= 'SELECT count(*) FROM ' . $db->nameQuote('#__community_events')
			...
			. ' AND ' . $db->nameQuote('id') . ' != ' . $db->Quote( $parent ) 
			. ' AND ' . $db->nameQuote('parent') . '=' . $db->Quote(0);
	 ...
	 */
	 public function getMonthlyEvents($month,$year){
		...
		$gid = JRequest::getVar( 'groupid','' , 'REQUEST'); //only display
		$query = '';
		//since 2.6, only display group events if groupid param exist in the GET
		if($gid){
			$query = "	SELECT DISTINCT DATE_FORMAT( ". $db->nameQuote('startdate').", '%d' ) AS date_start,
						DATE_FORMAT( ". $db->nameQuote('enddate').", '%d' ) AS date_end 
						FROM #__community_events
						WHERE DATE_FORMAT( ". $db->nameQuote('startdate').", '%Y%c' ) = ".$db->Quote( $year.$month )
						." AND ". $db->nameQuote('contentid')."=".$db->Quote( $gid );
		}else{
			$query = "	SELECT DISTINCT DATE_FORMAT( ". $db->nameQuote('startdate').", '%d' ) AS date_start,
						DATE_FORMAT( ". $db->nameQuote('enddate').", '%d' ) AS date_end 
						FROM #__community_events
	...
	public function getEventChilds($parent, $advance)
	{
		...
		if (isset($advance['id']) && $advance['id'] > 0) {
			$extraSQL .= ' AND ' . $db->nameQuote('id') .'> ' . $db->Quote($advance['id']);	
		}
		
		// Having id greater than or same
		if (isset($advance['eventid']) && $advance['eventid'] > 0) {
			$extraSQL .= ' AND ' . $db->nameQuote('id') .'>= ' . $db->Quote($advance['eventid']);	
		}
		
		// Having diffent id
		if (isset($advance['exclude'])) {
			$extraSQL .= ' AND ' . $db->nameQuote('id') .' NOT IN (' . $db->Quote($advance['exclude']) . ')';	
		}
		
		// non expired event
		if (isset($advance['expired']) && !$advance['expired']) {
			$pastDate  = CTimeHelper::getLocaleDate();
			$extraSQL .= ' AND ' . $db->nameQuote('enddate').' > ' . $db->Quote( $pastDate->toMySQL(true) );
		}
		
		// Published
		if (isset($advance['published'])) {
			$extraSQL .= ' AND ' . $db->nameQuote('published') .'= ' . $db->Quote($advance['published']);	
		}
	...
	public function updateRecurringImage( $path, $type, $parent, $id )
	{
		...
		$query	 = ' UPDATE ' . $db->nameQuote('#__community_events') .
	...
	{	
		...
		if ($series) {
			$extra = ' AND ' . $db->nameQuote('id') . ' < ' . $db->Quote( $id );
		}

		$strSQL	= ' SELECT count(*) 
				    FROM  ' . $db->nameQuote('#__community_events') . ' 
					WHERE ' . $db->nameQuote($type) . ' = ' . $db->Quote( $path ). ' AND '
						    . $db->nameQuote('id') . ' != ' . $db->Quote( $id ) . ' AND '
							. $db->nameQuote('published') . ' != ' . $db->Quote(2)
							. $extra . 
						  ' LIMIT 1';
