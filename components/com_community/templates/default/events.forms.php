...
<?php } ?>
	...
	<tr id="event-discription" style="display:none">
		...
		<td class="value">
			...
			<?php echo $editor->displayEditor( 'description',  $event->description , '10%', '10', '10', '10' , false); ?>
		...
		<td class="value">
			...
			<span class="jomNameTips" title="<?php echo JText::_('COM_COMMUNITY_EVENTS_LOCATION_TIPS');?>"><?php 
				...
				<select name="location" id="location">
				     ...
				     <option value="<?php echo $row[$ii][0];?>"<?php echo $event->location == $row[$ii][0] ? ' selected="selected"' : '';?>><?php echo $row[$ii][1]; ?></option>
	...
	<tr>			
		<td class="key">
			...
			<span class="jomNameTips" title="<?php echo JText::_('Please select Instructor');?>"><?php 
				$db = & JFactory::getDBO();
				$query = 'SELECT * FROM #__users WHERE 1 ORDER BY name';	
				$db->setQuery($query);
				$row = $db->loadRowList();?>
...
<!-- events Hourly Rate -->
	<tr>			
		<td class="key">
			...
			<span class="jomNameTips" title="<?php echo JText::_('Please enter the Hourly Rate in AUD');?>"><?php 
				$db = & JFactory::getDBO();
				$query = "SELECT * FROM pru_community_fields_values WHERE field_id=20 AND user_id=".$event->InstructorID; 	
				$db->setQuery($query);
				$row = $db->loadRowList();?>
				
				<input name="HourlyRate" title="<?php echo JText::_('Please enter the Hourly Rate in AUD'); ?>" id="HourlyRate" min="0" max="99999" step="1" size="5" type="number" size="30" class="inputbox jomNameTips" value="<?php echo $event->HourlyRate==0 ? '60':$event->HourlyRate;?>" />  
				</td>
	...
	<!-- events tickets -->
		...
		<td class="value">
			<input title="<?php echo JText::_('COM_COMMUNITY_EVENTS_NO_SEAT_DESCRIPTION'); ?>" name="ticket" id="ticket" type="hidden" size="10" maxlength="5" class="required inputbox jomNameTips" value="<?php echo (empty($event->ticket)) ? '0' : $this->escape($event->ticket); ?>" />
