...
CAssets::attach('assets/imgareaselect/scripts/jquery.imgareaselect.pack.js', 'js');
...
<?php 
				$db = & JFactory::getDBO();
				$query = 'SELECT * FROM #__locations WHERE ClubID=5 AND locid='.$this->escape($event->location);   	
				$db->setQuery($query);
				$row = $db->loadRowList();?>
				...
				<?php if( $config->get('eventshowmap') && ( $handler->isAllowed() || $event->permission != COMMUNITY_PRIVATE_EVENT ) ) {	?>
					...
					if(CMapping::validateAddress($row[0][2])){
						?>
						<div id="community-event-map" class="cModule">
							...
							<div class="app-box-content event-description">
								<!-- begin: dynamic map -->
								<?php echo CMapping::drawMap('event-map', $row[0][2]); ?>
								<div id="event-map" style="height:210px;width:100%;margin:5px 0;">
									<?php echo JText::_('COM_COMMUNITY_MAPS_LOADING'); ?>
								</div>
								<!-- end: dynamic map -->
								<?php echo CMapping::getFormatedAdd($row[0][2]); ?>
							</div>
							<div class="app-box-footer">
								<a href="http://maps.google.com/?q=<?php echo urlencode($row[0][2]); ?>" target="_blank"><?php echo JText::_('COM_COMMUNITY_EVENTS_FULL_MAP'); ?></a>
							</div>
						</div>
					<?php } ?>
				<?php } ?>
...
<?php 
				...
				$row = $db->loadRowList();?>
								...
								<div class="cdata" id="community-event-data-location"><a href="http://maps.google.com/?q=<?php echo urlencode($row[0][2]); ?>" target="_blank"><?php echo $row[0][2]; ?></a></div>
							</div>

<!-- Instructor info -->
<div class="InstructorID">
<div class="clabel">Instructor:</div>
			...
			<div><?php 
				...
				$db = & JFactory::getDBO();
				$query = 'SELECT * FROM #__users WHERE id='.$event->InstructorID;
			...
			?></div>
							...
							<div class="cparam event-summary">
								...
								<div class="cdata"><?php echo JText::_('COM_COMMUNITY_EVENTS_REPEAT_' . strtoupper($event->repeat)); ?></div>
							</div>
							<?php }?>
