	...
	<tr>
		...
		<td class="value">
			...
			<span class="jomNameTips" title="<?php echo JText::_('COM_COMMUNITY_EVENTS_LOCATION_TIPS');?>"><?php 
				$db = & JFactory::getDBO();
				$query = 'SELECT * FROM #__locations WHERE 1';	
				$db->setQuery($query);
				$row = $db->loadRowList();?>
			
				<select name="location" id="location">
				     <?php $ii=0; foreach ( $row as $option ) : ?>
				     <option value="<?php echo $row[$ii][2]; ?>"><?php echo $row[$ii][1]; ?></option>
		...
		<td class="key">
			...
			<span class="jomNameTips" title="<?php echo JText::_('Please select Instructor');?>"><?php 
				...
				<select name="InstructorID">
				     <?php $ii=0; foreach ( $row as $option ) : ?>
				     <option value="<?php echo $row[$ii][0]; ?>"><?php echo $row[$ii][1]; ?></option>
				     <?php $ii=$ii+1; endforeach; ?>
				</select>
	...
	<tr>			
		<td class="key">
			...
			<span class="jomNameTips" title="<?php echo JText::_('Please enter the Hourly Rate in AUD');?>"><?php 
				...
				<td class="value">
					<input name="HourlyRate" title="<?php echo JText::_('Please enter the Hourly Rate in AUD'); ?>" id="HourlyRate" type="text" size="40" maxlength="14" class="required inputbox jomNameTips" value="<?php echo $this->escape($event->HourlyRate); ?>" />
				</td>
