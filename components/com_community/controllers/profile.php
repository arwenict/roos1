...
class CommunityProfileController extends CommunityBaseController
{
	...
	public function ajaxPlayProfileVideo($videoid=null, $userid=0)
	{
		...
		if(!empty($video->id))
		{
			...
			else
			{
				...
				$content	=	$notiHtml = '<div class="video-player">
								'.$video->getPlayerHTML().'
								</div>';
	...
	public function edit()
	{
		...
		{
			if($this->_saveProfile()) $this->save();
		}
	...
	{		
		...
		$mainframe->redirect(CRoute::_('index.php?option=com_community&view=profile&task=edit', false), '', '');
		/*
	...
	public function preferences()
	{
		...
		{			
			...
			if( empty($saveSuccess) || !in_array( false , $saveSuccess ) )
			{
				...
				if(isset($postvars['alias'])) unset($postvars['alias']);
				if(isset($postvars['activityLimit'])) unset($postvars['alias']);
				if(isset($postvars['profileLikes'])) unset($postvars['alias']);
	...
	public function privacy()
	{
		...
		$mainframe	=& JFactory::getApplication();
		$mainframe->redirect(CRoute::_('index.php?option=com_community&view=profile&task=preferences', false), '', '');
		/*
		if($my->id == 0)
		{
		   return $this->blockUnregister();
	...
	public function ajaxShowProfileFeatured( $userId ){
		...
		$photoModel = CFactory::getModel('photos');
		$photoList2 = $photoModel->getAlbums($userId);
		
		$photoList = '';
		if(count($photoList2)>0){
		    $photos = array_rand($photoList2,4);

		    $photoList='<div id="cPhotoItems" class="photo-list-item">
					<p><strong>'.JText::_('COM_COMMUNITY_NOTIFICATIONGROUP_PHOTOS').':</strong></p>';

		    for($i=0; $i<4; $i++){
			if($photoList2[$photos[$i]]->photoid!=""){
			$photoList .= '<div class="cPhotoItem">
						<a href="'.CRoute::_('index.php?option=com_community&view=photos&task=photo&userid='.$photoList2[$photos[$i]]->creator.'&albumid='.$photoList2[$photos[$i]]->id.'#photoid='.$photoList2[$photos[$i]]->photoid).'"><img src="'.$photoList2[$photos[$i]]->thumbnail.'" id="photoid-'.$photoList2[$photos[$i]]->photoid.'" /></a>
					</div>';
			}
		    }
