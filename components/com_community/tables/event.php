	...
	implements  CGeolocationInterface, CTaggable_Item
{
	...
	public function deleteAllMembers($action = '')
	{
		...
		if ($action == 'future') {
			$condition .= ' OR
				           (e.' . $db->nameQuote('parent') . '= ' . $db->Quote($this->parent).' &&
						    e.' . $db->nameQuote('parent') . '!= 0 &&
						    e.' . $db->nameQuote('id') . '>' . $db->Quote($this->id) . '
						    )';
		}
		
		$query	= 'DELETE
			       FROM m
				   USING ' . $db->nameQuote('#__community_events_members') . ' AS m 
				   LEFT JOIN ' . $db->nameQuote('#__community_events') . ' AS e on e.id = m.eventid
				   WHERE m.' . $db->nameQuote( 'eventid' ) . '=' . $db->Quote( $this->id ) . 
						 $condition;
	...
	public function deleteWalls()
	{
		...
		$sql = "DELETE
				...
				FROM w
				USING ".$db->nameQuote("#__community_wall")." AS w
				LEFT JOIN ".$db->nameQuote("#__community_activities")." AS a 
					 ON a.id = w.contentid
				WHERE
					 w." . $db->nameQuote("type")  . " = ".$db->quote("events.wall") . " AND
				     a." . $db->nameQuote("cid")   . " = " . $this->id . " AND 
				     a." . $db->nameQuote("app")   . " = " . $db->quote("events.wall");
	...
	public function getMembers( $status, $limit = 0, $randomize = false, $pendingApproval = false )
	{
		...
		$query	= 'SELECT ' . $this->_db->nameQuote('memberid') . ' AS id, a.' . $this->_db->nameQuote('status') . ' AS statusCode FROM '
				. $this->_db->nameQuote('#__community_events_members').' AS a '
	...
	{		
		$date	= JFactory::getDate( $this->repeatend );
		return $date;
	}
