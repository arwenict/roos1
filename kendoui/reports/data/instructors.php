...
if ($verb == "GET") {
	...
	$arr = array();
	$rs = mysql_query("SELECT u.id, name, username, email, cfvm.value as mobile, cfvs.value as skills FROM pr_users u LEFT JOIN pr_community_fields_values cfvm on u.id=cfvm.user_id AND cfvm.field_id=6 LEFT JOIN pr_community_fields_values cfvs on u.id=cfvs.user_id AND cfvs.field_id=19");
